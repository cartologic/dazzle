...
class WidgetFrame extends Component {
   render() {
     ...
     let height = null;
     try {
       height = layout.rows[rowIndex].columns[columnIndex].tabs[tabIndex].widgetSizes[widgetIndex];
     } catch (e) {
       const percentage = 100 / layout.rows[rowIndex].columns[columnIndex].tabs[tabIndex].widgets.length;
  ...
  edit = () => {
    const { layout, rowIndex, columnIndex, tabIndex, widgetIndex } = this.props;
    this.props.onEdit(layout.rows[rowIndex].columns[columnIndex].tabs[tabIndex].widgets[widgetIndex].key);
  }

  remove = () => {
    const { layout, rowIndex, columnIndex, tabIndex, widgetIndex } = this.props;
    const newLayout = removeWidget(layout, rowIndex, columnIndex, tabIndex, widgetIndex);
    this.props.onRemove(newLayout, rowIndex, columnIndex, tabIndex, widgetIndex);
  }
 }
