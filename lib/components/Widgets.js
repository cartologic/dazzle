 ...
 */
/* eslint max-len: "off" */
const Widgets = ({ widgets, widgetTypes, onRemove, layout, columnIndex, rowIndex, tabIndex, editable, frameComponent, onMove, containerClassName, onEdit }) => {
  const createdWidgets = widgets.map((widget, index) => { // eslint-disable-line arrow-body-style
    return (
      <WidgetFrame
        key={'widgetframe-' + rowIndex.toString() + columnIndex.toString() + tabIndex.toString() + index.toString()}
  ...
  return <div style={style} className={containerClassName}>{createdWidgets}</div>;
};
